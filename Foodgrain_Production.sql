...
#1: Total Food Grain  Production of India

SELECT ROUND(SUM(Production_Foodgrains)) AS TOTAL_PRODUCTION_TONNES
FROM FOOD;

#2:	What Is The Average Rainfall Across All States?

SELECT ROUND(AVG(Rainfall_mm)) AS AVERAGE_RAINFALL FROM FOOD;

#3:	Which State Has The Highest Food Grain Production?
...
#4:	Which Three State Has The Lowest Fertilizer Usage?

SELECT State, Fertilizer_Usage FROM FOOD
ORDER BY Fertilizer_Usage LIMIT 3;

#5:	What Is The Total Area Under Production For All States Combined?

SELECT SUM(Area_Under_Production) AS TOTAL_AREA FROM FOOD;

#6:	Calculate The Average Food Grain Production Per Hectare For Each State.

SELECT State, AVG(Production_Foodgrains/Area_Under_Production) AS AVERAGE_PRODUCTION_PER_HECTARE FROM FOOD
GROUP BY State;

#7:	Find The Top 5 States With The Highest Rainfall

SELECT State, Rainfall_mm FROM FOOD
ORDER BY Rainfall_mm DESC LIMIT 5;

#8:	Determine The States With Food Grain Production Above The National Average.
...
#9 The Maximum and Minimum Rainfall Recorded Among All States.

SELECT State, MAX(Rainfall_mm) AS Max_Rainfall, MIN(Rainfall_mm) AS MIN_Rainfall FROM FOOD
GROUP BY State;

#10	Which States Have A Fertilizer Usage Higher Than The Average Fertilizer Usage And A Food Grain Production Lower Than The Average Food Grain Production?
...
#11: Creating a New Column for Rainfall Categories
...
#12: Rank States Based On Foodgrain Production:

SELECT State, Production_Foodgrains,
RANK() OVER (ORDER BY Production_Foodgrains DESC) AS Production_Rank
FROM FOOD;

#13: Generate A New Column That Categorizes States Based On Area Under Production (Small, Medium, Large)
...
#14: Group States By Area Category(Low, Medium, High).
...
#15: Find The States With Total Fertilizer Usage For The 'Low' Area Category:	
